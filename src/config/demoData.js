...
// Purchase payments MVP configuration and demo data
export const DEMO_CONFIG_PURCHASE_PAYMENTS_MVP = {
  enabled: true,
  useRealAPI: false,
  delayMs: 650,
  pageSize: 10,
...
export const DEMO_PURCHASE_PAYMENTS_MVP_ORDERS = [
  {
    id: 'PO-2025-0101',
    supplier: {
      id: 101,
      name: 'Distribuidora Nacional S.A.',
      contact: '+595 21 555 000',
    },
    status: 'pending',
...
export const getDemoPurchasePaymentsMvpOrders = async (params = {}) => {
  await simulateDelay(DEMO_CONFIG_PURCHASE_PAYMENTS_MVP.delayMs)
  ...
  const normalizedSupplier =
    supplierId === undefined || supplierId === null ? 'all' : String(supplierId)
  const normalizedStatus =
    status === undefined || status === null ? 'all' : String(status)

  let filteredOrders = [...DEMO_PURCHASE_PAYMENTS_MVP_ORDERS]
  ...
  if (normalizedSupplier && normalizedSupplier !== 'all') {
    filteredOrders = filteredOrders.filter(
      order => String(order?.supplier?.id) === normalizedSupplier
    )
  }
  ...
  if (dateFrom) {
    ...
    filteredOrders = filteredOrders.filter(
      order => new Date(order.issue_date).getTime() >= fromTs
    )
  }
  ...
  if (dateTo) {
    ...
    filteredOrders = filteredOrders.filter(
      order => new Date(order.issue_date).getTime() <= toTs
    )
  }
  ...
  const supplierOptions = Array.from(
    new Map(
      DEMO_PURCHASE_PAYMENTS_MVP_ORDERS.map(order => {
        const value = String(order?.supplier?.id ?? '')
        return [
          value,
          {
            value,
            label: order?.supplier?.name ?? value,
          },
        ]
      })
    ).values()
  ...
  const statusOptions = Array.from(
    new Set(DEMO_PURCHASE_PAYMENTS_MVP_ORDERS.map(order => order.status))
  ).map(value => ({ value }))
  ...
  return {
    ...
    meta: {
      page: safePage,
      pageSize:
        parseInt(pageSize, 10) || DEMO_CONFIG_PURCHASE_PAYMENTS_MVP.pageSize,
      total,
      totalPages,
    },
    filters: {
