...
const normalizeScheduleList = payload => {
  if (!payload) return []
...
const ensureSchedulePayload = (payload, schedules) => {
  const normalizedList = Array.isArray(schedules)
    ? schedules
    : normalizeScheduleList(payload)

  const normalizedCount =
    typeof payload?.count === 'number' ? payload.count : normalizedList.length

  return {
    ...(Array.isArray(payload) ? {} : payload),
    data: payload?.data ?? normalizedList,
    schedules: payload?.schedules ?? normalizedList,
...
export const scheduleService = {
  ...
  // Método deprecado - usar generateForDate con options
  async generateForDateWithCustomRange(
    targetDate,
    startHour,
    endHour,
    productIds = null
  ...
  async getAvailableSchedules(productId, date) {
    ...
    try {
      let rawResult = await withRetry(async () => {
        return await apiService.get(
          `${API_PREFIX}/product/${productId}/date/${date}/available`
        )
      })

      let schedules = normalizeScheduleList(rawResult)
      ...
      if (schedules.length === 0) {
        ...
        rawResult = await apiService.get(
          `${API_PREFIX}/product/${productId}/date/${date}/available`
        )
        schedules = normalizeScheduleList(rawResult)
      ...
      return ensureSchedulePayload(rawResult, schedules)
    } catch (error) {
      telemetry.record('schedules.service.error', {
        duration: Date.now() - startTime,
        error: error.message,
  ...
  async getAllSchedulesByProductAndDate(productId, date) {
    ...
    try {
      const rawResult = await withRetry(async () => {
        return await apiService.get(
          `${API_PREFIX}/product/${productId}/date/${date}/all`
        )
      })

      const schedules = normalizeScheduleList(rawResult)

      telemetry.record('schedules.service.load_all_by_product_and_date', {
        duration: Date.now() - startTime,
        count: schedules.length,
        available_count: schedules.filter(s => s.is_available).length,
        unavailable_count: schedules.filter(s => !s.is_available).length,
      })
  ...
  async getSchedulesByProduct(productId, params = {}) {
    ...
    try {
      ...
      const rawResult = await withRetry(async () => {
        return await apiService.get(
          `${API_PREFIX}/product/${productId}?${queryParams.toString()}`
        )
      })
  ...
  async checkSchedulesForDate(date, productId = null) {
    ...
    try {
      if (productId) {
        ...
        const schedules = normalizeScheduleList(result)
        return {
        ...
        const count =
          typeof result?.count === 'number'
            ? result.count
            : normalizeScheduleList(result).length
        return {
          hasSchedules: count > 0,
          count,
          message: result?.message || 'Sin información de horarios',
        }
      }
    } catch (error) {
      telemetry.record('schedules.service.error', {
