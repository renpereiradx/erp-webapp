...
const isDemoMode = () => {
  return import.meta.env.VITE_APP_MODE === 'demo' || import.meta.env.MODE === 'demo' || true; // Forzamos true para desarrollo inicial si no hay API
};
...
const _handleRequest = async (endpoint, params = {}, mockFallback) => {
  if (isDemoMode()) {
    console.log(`[InventoryAnalyticsService] Demo Mode: Returning mock data for ${endpoint}`);
  ...
  try {
    ...
    console.error(`[InventoryAnalyticsService] Error fetching ${endpoint}:`, error);
    
    // Fallback si la API no existe aún (404)
    if (error.status === 404 || error.response?.status === 404) {
      console.warn(`[InventoryAnalyticsService] Endpoint ${endpoint} not found. Returning mock data.`);
      return { success: true, data: mockFallback };
    }
...
export const inventoryAnalyticsService = {
  getOverview: () => _handleRequest(API_ENDPOINTS.overview, {}, mockData.mockInventoryOverview),
  
  getStockLevels: (params = {}) => _handleRequest(API_ENDPOINTS.stockLevels, params, mockData.mockStockLevels),
  
  getTurnover: (period = 'month', limit = 20) => 
    _handleRequest(API_ENDPOINTS.turnover, { period, limit }, mockInventoryABCData),
    
  getABC: (period = 'month') => _handleRequest(API_ENDPOINTS.abc, { period }, mockData.mockInventoryDashboard),
  
  getDeadStock: (daysThreshold = 90) => 
    _handleRequest(API_ENDPOINTS.deadStock, { days_threshold: daysThreshold }, mockData.mockDeadStockAnalysis),
    
  getReorderAnalysis: () => _handleRequest(API_ENDPOINTS.reorder, {}, mockData.mockReorderAnalysis),
  
  getForecast: (forecastDays = 30) => 
    _handleRequest(API_ENDPOINTS.forecast, { forecast_days: forecastDays }, mockData.mockStockForecast),
    
  getDashboard: () => _handleRequest(API_ENDPOINTS.dashboard, {}, mockData.mockInventoryDashboard),
};
