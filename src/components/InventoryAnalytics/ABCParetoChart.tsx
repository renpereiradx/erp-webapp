...
export const ABCParetoChart: React.FC<ABCParetoChartProps> = ({
  ...
  const classLabels: Record<string, string> = {
    A: t('bi.inventory.abc.classA', 'Clase A', {}),
    B: t('bi.inventory.abc.classB', 'Clase B', {}),
    C: t('bi.inventory.abc.classC', 'Clase C', {}),
  };

  return (
    <div className="flex flex-col gap-6 bg-surface rounded-xl p-6 border border-border-subtle shadow-sm font-display">
      <div className="flex items-center justify-between">
        <h2 className="text-foreground text-xl font-bold uppercase tracking-tight">{t('bi.inventory.abc.title', 'Desglose ABC de Valor', {})}</h2>
        <span className="px-2 py-1 bg-primary text-white text-[10px] font-bold rounded uppercase tracking-tighter font-mono">{t('bi.inventory.abc.pareto', 'Pareto Real', {})}</span>
      </div>
      ...
      <div className="h-48 w-full flex items-end gap-2 px-2 border-b border-l border-border-subtle relative">
        ...
        >
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-inverse-background text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap font-mono z-10 shadow-fluent-8">
            {classLabels.A}: {formatNumber(classAPct)}%
          </div>
        </div>
        <div
        ...
        >
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-inverse-background text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap font-mono z-10 shadow-fluent-8">
            {classLabels.B}: {formatNumber(classBPct)}%
          </div>
        </div>
        <div
        ...
        >
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-inverse-background text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap font-mono z-10 shadow-fluent-8">
            {classLabels.C}: {formatNumber(classCPct)}%
          </div>
        </div>
      ...
      <div className="flex justify-between text-[10px] font-bold text-on-surface-deep px-2 font-mono">
        <span>{classLabels.A.toUpperCase()}</span>
        <span>{classLabels.B.toUpperCase()}</span>
        <span>{classLabels.C.toUpperCase()}</span>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
          ...
          {t('bi.inventory.abc.topProducts', 'Productos Top Clase A', {})}
        </h3>
        <div className="space-y-3">
            ...
            <div key={product.id || product.product_id || idx} className="flex items-center justify-between p-3 rounded-lg bg-surface-muted border border-border-subtle">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-on-surface-deep">
                  {t('bi.inventory.abc.shareOfValue', 'Representa el {pct}% del valor total', {
                    pct: formatNumber(product.percentage || product.sales_percentage || 0),
                  })}
                </span>
              </div>
          ...
          {(!classAProducts || classAProducts.length === 0) && (
            <p className="text-sm text-on-surface-deep text-center py-4 italic">{t('bi.inventory.abc.empty', 'No hay productos Clase A registrados.', {})}</p>
          )}
        </div>
      </div>
    </div>
