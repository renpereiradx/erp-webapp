...
const PaymentMethodSelector = ({
  ...
  const usesExternalData = typeof externalPaymentMethods !== 'undefined'
  ...
  }, [filterByType, usesExternalData])

  useEffect(() => {
    if (usesExternalData) {
      setPaymentMethods(externalPaymentMethods || [])
  ...
  const resolvedPaymentMethods = useMemo(() => {
    return usesExternalData ? externalPaymentMethods || [] : paymentMethods
  }, [usesExternalData, externalPaymentMethods, paymentMethods])
  ...
  const filteredMethods = resolvedPaymentMethods.filter(method => {
  ...
  const selectedMethod = resolvedPaymentMethods.find(method => {
    if (method == null) {
      return false
    }
    const candidateId = method.id ?? method.payment_method_id
    if (candidateId == null || value == null) {
      return false
    }
    return String(candidateId) === String(value)
  })
          ...
          {selectedMethod ? (
            <>
              <div className='flex items-center mr-3'>
                ...
                <span
                  className={`ml-2 inline-block px-2 py-1 text-xs font-mono font-bold text-white rounded ${getMethodBadgeColor(
