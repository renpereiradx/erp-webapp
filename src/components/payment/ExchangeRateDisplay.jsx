...
const ExchangeRateDisplay = ({
  ...
  const failedCurrenciesRef = useRef(failedCurrencyCache)
  ...
  useEffect(() => {
    ...
    const currencyKey = String(currencyId)
    const hasPermanentFailure = failedCurrenciesRef.current.has(currencyKey)
  ...
  const loadExchangeRate = async () => {
    ...
    const currencyKey = String(currencyId)
    if (failedCurrenciesRef.current.has(currencyKey)) {
      setIsLoading(false)
      setError(
    ...
    try {
      ...
      failedCurrenciesRef.current.delete(currencyKey)

      // Validate rate
      const validationResult = await ExchangeRateService.validateExchangeRate(
        currencyId,
        date
      )
      ...
      if (showTrend && rate) {
        try {
          ...
          console.warn(
            `Previous exchange rate not available for ${currencyId} on previous date, hiding trend:`,
            err?.message || err
          )
      ...
      const alreadyMarked = currencyId
        ? failedCurrenciesRef.current.has(currencyKey)
        : false
      ...
      const message = err?.message || 'Sin tipo de cambio disponible'
      setError(message)

      if (message.includes('no está disponible')) {
        failedCurrenciesRef.current.add(currencyKey)
      }
    } finally {
      setIsLoading(false)
    }
  ...
  const handleRefresh = () => {
    ...
    if (currencyId) {
      failedCurrenciesRef.current.delete(String(currencyId))
    }
    setError(null)
    setValidation(null)
    setIsLoading(true)
  ...
  if (isLoading) {
    return (
      <div
        className={`border border-gray-200 rounded-lg bg-gray-50 ${sizes.container} ${className}`}
      >
  ...
  if (error || !exchangeRate || !currency) {
    return (
      <div
        className={`border border-red-200 rounded-lg bg-red-50 ${sizes.container} ${className}`}
      >
        <div className='flex items-center'>
          <AlertTriangle className={`${sizes.icon} text-red-500 mr-2`} />
          <span className='text-red-600'>
            {error || 'Error al cargar tipo de cambio'}
          </span>
        </div>
      </div>
    )
  ...
  return (
    <div
      className={`border border-gray-200 rounded-lg bg-white ${sizes.container} ${className}`}
    >
      ...
      <div className='flex items-center justify-between mb-2'>
        <div className='flex items-center'>
          ...
          {showRefresh && (
            ...
            >
              <RefreshCw
                className={`${sizes.icon} ${isLoading ? 'animate-spin' : ''}`}
              />
            </button>
          )}
        </div>
        ...
        {showTrend && trendInfo.trend !== 'neutral' && (
          <div className={`flex items-center ${sizes.code}`}>
            ...
            <span
              className={
                trendInfo.trend === 'up' ? 'text-green-600' : 'text-red-600'
              }
            >
