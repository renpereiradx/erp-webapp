...
export const AttributesTab: React.FC<AttributesTabProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-gutter h-full">
      ...
      <div className="lg:col-span-2 bg-surface rounded-[12px] p-lg shadow-sm border border-divider/30 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
          <table className="w-full border-separate border-spacing-0 text-left">
            <thead className="sticky top-0 bg-surface-muted z-10 rounded-t-[12px]">
              <tr>
                <th className="font-label-caps text-label-caps text-on-surface-deep py-sm px-md rounded-tl-[12px]">Nombre</th>
                <th className="font-label-caps text-label-caps text-on-surface-deep py-sm px-md">Código</th>
                <th className="font-label-caps text-label-caps text-on-surface-deep py-sm px-md">Tipo</th>
                <th className="font-label-caps text-label-caps text-on-surface-deep py-sm px-md text-center">Req.</th>
                <th className="font-label-caps text-label-caps text-on-surface-deep py-sm px-md text-center">Filt.</th>
                <th className="font-label-caps text-label-caps text-on-surface-deep py-sm px-md rounded-tr-[12px]">Variante</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-divider/10">
              {attributes.map(attr => (
                <tr 
                  ...
                  className={`transition-colors duration-150 cursor-pointer group ${selectedAttribute?.id === attr.id ? 'bg-surface-muted/50' : 'hover:bg-surface-muted/50'}`}
                >
                  <td className="py-md px-md font-body-md text-foreground font-medium">{attr.name}</td>
                  ...
                  <td className="py-md px-md text-center">
                    <span className={`material-symbols-outlined text-[20px] ${attr.isRequired ? 'text-primary' : 'text-divider'}`}>
                      {attr.isRequired ? 'check_box' : 'check_box_outline_blank'}
                    </span>
                  </td>
                  <td className="py-md px-md text-center">
                    <span className={`material-symbols-outlined text-[20px] ${attr.isFilterable ? 'text-primary' : 'text-divider'}`}>
      ...
      <div className="bg-surface rounded-[12px] p-lg shadow-sm border border-divider/30 flex flex-col h-full">
        <div className="flex justify-between items-center mb-md border-b border-divider/20 pb-sm">
          <h3 className="font-title-md text-title-md text-foreground">Editor de Atributo</h3>
        ...
        {selectedAttribute ? (
          <form onSubmit={handleSave} className="flex-1 overflow-y-auto pr-xs space-y-md custom-scrollbar">
            ...
            <div className="grid grid-cols-2 gap-sm">
              <div>
                ...
                <div className="relative">
                  ...
                  <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-deep pointer-events-none">expand_more</span>
                </div>
              </div>
              <div>
            ...
            {formData.type === 'LIST' && (
              <div className="bg-surface-muted p-sm rounded-lg border border-divider/30">
                <label className="block font-body-sm-bold text-body-sm-bold text-foreground mb-sm flex justify-between items-center">
                  Opciones de Lista
            ...
            <div className="grid grid-cols-2 gap-y-sm gap-x-md pt-sm border-t border-divider/20">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input 
                  checked={!!formData.isRequired} 
                  onChange={(e) => handleChange('isRequired', e.target.checked)}
                  className="rounded text-primary border-divider focus:ring-primary w-4 h-4 bg-surface" 
                  type="checkbox" 
                />
                <span className="font-body-md text-foreground">Requerido</span>
              </label>
              <label className="flex items-center space-x-2 cursor-pointer">
                <input 
                  ...
                  type="checkbox" 
                />
                <span className="font-body-md text-foreground">Filtrable</span>
              </label>
              <label className="flex items-center space-x-2 cursor-pointer">
                <input 
                  ...
                  type="checkbox" 
                />
                <span className="font-body-md text-foreground">Visible</span>
              </label>
              <label className="flex items-center space-x-2 cursor-pointer">
                <input 
                  checked={!!formData.isVariant}
                  onChange={(e) => handleChange('isVariant', e.target.checked)}
                  className="rounded text-[#059669] border-divider focus:ring-[#059669] w-4 h-4 bg-surface" 
                  type="checkbox" 
                />
            ...
            <div className="pt-md mt-auto border-t border-divider/20 flex justify-end space-x-sm shrink-0">
              <button onClick={() => onSelectAttribute(null)} type="button" className="px-md py-sm rounded bg-surface-muted text-foreground font-body-sm-bold hover:bg-surface-deep transition-colors disabled:opacity-50" disabled={loading}>Cancelar</button>
