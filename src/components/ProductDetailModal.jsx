  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <div className="flex items-center gap-2">
        ...
        <h3 className={`${styles.header('h3')} text-base font-semibold`}>{title}</h3>
      </div>
      <div className={`space-y-2 p-3 rounded-lg border ${styles.card('subtle')}`}>
        {children}
      </div>
    </div>
  );
...
const DetailRow = ({ label, value, type = 'text' }) => {
  const renderValue = () => {
    ...
    switch (type) {
      case 'boolean':
        return (
          <Badge variant={value ? 'default' : 'secondary'} className="flex items-center gap-1 product-badge">
      ...
      case 'stock_status':
        const statusConfig = {
          ...
          'out_of_stock': { variant: 'destructive', text: 'Sin Stock', icon: XCircle },
          'low_stock': { variant: 'secondary', text: 'Stock Bajo', icon: AlertTriangle }
        };
        const config = statusConfig[value] || { variant: 'outline', text: value, icon: Info };
        const StatusIcon = config.icon;
        
        return (
          <Badge variant={config.variant} className={`flex items-center gap-1 product-badge ${
            value === 'in_stock' ? 'validation-success' : 
            value === 'out_of_stock' ? 'validation-error' : 
            value === 'low_stock' ? 'validation-warning' : ''
          }`}>
...
const ProductDetailModal = ({ isOpen, onClose, product, container = null }) => {
  ...
  return (
    <div className={`${container ? 'absolute' : 'fixed'} inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200 product-detail-modal ${container ? 'modal-in-container' : ''}`}>
      <div className={`w-full max-w-3xl max-h-[85vh] flex flex-col modal-content ${styles.card()} shadow-2xl animate-in slide-in-from-bottom-4 duration-300`}>
        {/* Header */}
        ...
        <ScrollArea className="flex-1 p-4 product-scroll-area">
          <Tabs defaultValue="general" className="w-full">
            <TabsList className="grid w-full grid-cols-4 mb-4 tab-list">
              <TabsTrigger value="general" className="flex items-center gap-2 product-tab tab-trigger">
                <Info className="h-4 w-4" />
                General
              </TabsTrigger>
              <TabsTrigger value="description" className="flex items-center gap-2 product-tab tab-trigger">
                <FileText className="h-4 w-4" />
                Descripción
              </TabsTrigger>
              <TabsTrigger value="pricing" className="flex items-center gap-2 product-tab tab-trigger">
                <DollarSign className="h-4 w-4" />
                Precios
              </TabsTrigger>
              <TabsTrigger value="inventory" className="flex items-center gap-2 product-tab tab-trigger">
            ...
            <div className="space-y-4">
                ...
                {processedProduct.categoryDescription && (
                  <DetailSection title="Categoría" icon={Tag}>
                    <div className="text-sm text-muted-foreground">
                      ...
                      {processedProduct.categoryDescription && (
                        <p className="mt-1">{processedProduct.categoryDescription}</p>
                      )}
                    </div>
                  </DetailSection>
                )}
              ...
              <TabsContent value="description" className="space-y-4 tab-content">
                <DetailSection title="Descripción del Producto" icon={FileText}>
                  {processedProduct.description ? (
                    <div className="space-y-3">
                      <p className="text-sm leading-relaxed">{processedProduct.description}</p>
              ...
              <TabsContent value="pricing" className="space-y-4 tab-content">
                <DetailSection title="Información de Precios" icon={DollarSign}>
                  ...
                  {(processedProduct.price_id || processedProduct.priceUpdatedAt || processedProduct.price_updated_by) && (
                    <>
                      <Separator />
                      <div className="pt-3 space-y-2 text-xs text-muted-foreground">
                        ...
                        {processedProduct.price_id && (
                          <DetailRow label="ID de Precio" value={processedProduct.price_id} />
                        )}
                        {processedProduct.priceUpdatedAt && (
                          <DetailRow label="Última Actualización" value={processedProduct.priceUpdatedAt} />
                        )}
                        {processedProduct.price_updated_by && (
                          <DetailRow label="Actualizado Por" value={processedProduct.price_updated_by} />
                        )}
                      </div>
                    </>
                  )}
              ...
              <TabsContent value="inventory" className="space-y-4 tab-content">
                <DetailSection title="Información de Inventario" icon={Layers}>
                  ...
                  {(processedProduct.stock_id || processedProduct.stockUpdatedAt || processedProduct.stock_updated_by) && (
                    <>
                      <Separator />
                      <div className="pt-3 space-y-2 text-xs text-muted-foreground">
                        <h4 className="font-medium">Metadatos de Inventario:</h4>
                        {processedProduct.stock_id && (
                          <DetailRow label="ID de Stock" value={processedProduct.stock_id} />
                        )}
                        {processedProduct.stockUpdatedAt && (
                          <DetailRow label="Última Actualización" value={processedProduct.stockUpdatedAt} />
                        )}
                        {processedProduct.stock_updated_by && (
                          <DetailRow label="Actualizado Por" value={processedProduct.stock_updated_by} />
                        )}
                      </div>
                    </>
                  )}
