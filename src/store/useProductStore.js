  ...
  devtools(
    (set, get) => ({
      ...
      _recordSuccess: () => {
        const state = get()
        try {
          if (state.circuitTimeoutId) {
            clearTimeout(state.circuitTimeoutId)
          }
        } catch (e) {}
      ...
      _circuitOpen: () => {
        ...
        if (openUntil && Date.now() >= openUntil) {
          try {
            if (state.circuitTimeoutId) clearTimeout(state.circuitTimeoutId)
          } catch (e) {}
      ...
      _closeCircuit: (reason = 'manual') => {
        ...
        try {
          telemetry.record('circuit.close', { component: 'products', reason })
        } catch {}
      },
      ...
      _withRetry: async (fn, opts = {}) => {
        ...
        for (let attempt = 0; attempt < attempts; attempt++) {
          try {
            ...
            try {
              if (telemetryKey)
                telemetry.record(`${telemetryKey}.retry`, {
                  attempt: attempt + 1,
                })
            } catch (e) {}
      ...
      refreshCategoriesFromAPI: async () => {
        ...
        try {
          const categories = await categoryCacheService.forceRefreshFromAPI(
            apiClient
          )
          ...
          if (currentState.categories.length > 0) {
            set({
              loading: false,
              error: 'No se pudo actualizar, usando caché local',
            })
            return currentState.categories
          }

          const fallbackCategories =
            categoryCacheService.getFallbackCategories()
          set({
            categories: fallbackCategories,
            loading: false,
      ...
      fetchProducts: async (
        ...
        const correlationId = `fetch_${Date.now()}_${Math.random()
          ...
          .slice(2, 8)}`
        try {
          if (process && process.env && process.env.DEBUG_FETCH)
            console.debug('[store] fetchProducts called', {
              page,
              pageSize,
              searchTerm,
            })
        ...
        try {
          ...
          if (searchTerm && searchTerm.trim()) {
            ...
            if (totalCount === 0) {
              ...
              return {
                data: [],
                total: 0,
                message: `No se encontraron productos para "${searchTerm.trim()}"`,
              }
            }
          ...
          try {
            telemetry.record('products.error.store', {
      ...
      searchProducts: async (searchTerm, options = {}) => {
        ...
        try {
          ...
          if (isValidCache && !options.force) {
            ...
            telemetry.record('products.search.cache.hit', {
              term,
              age: now - cached.ts,
            })
            ...
            const byId = Object.fromEntries(
              cached.data.map(p => [p.product_id || p.id, p])
            )
            set({
              products: paginated,
              productsById: byId,
              ...
              ;(async () => {
                try {
                  ...
                  set(s => ({
                    searchCache: {
                      ...s.searchCache,
                      [cacheKey]: { ts: Date.now(), data: freshArr },
                    },
                  }))
                  telemetry.record('products.search.revalidate.auto.success', {
                    term,
                    total: freshArr.length,
                  })
                  ...
                  if (freshArr.length !== totalCount) {
                    ...
                    const freshById = Object.fromEntries(
                      freshArr.map(p => [p.product_id || p.id, p])
                    )
                    set({
                      products: pag,
                      productsById: freshById,
                      totalProducts: freshArr.length,
                      totalPages: Math.ceil(freshArr.length / pageSize),
                    })
                  }
                } catch {
                  telemetry.record('products.search.revalidate.auto.error', {
                    term,
                  })
                }
              })()
          ...
          if (options.signal) {
            ...
            set(s => ({
              searchCache: {
                ...s.searchCache,
                [cacheKey]: { ts: Date.now(), data: products },
              },
            }))
          ...
          set(s => ({
            errorCounters: {
              ...s.errorCounters,
              [norm.code]: (s.errorCounters[norm.code] || 0) + 1,
            },
          }))
          ...
          telemetry.record('products.search.error', {
            code: norm.code,
          ...
          try {
            if (isConnectionError(error)) set({ isOffline: true })
          } catch {}
      ...
      loadPage: async page => {
        ...
        if (
          ...
          Date.now() - state.pageCache[page].ts < ttl
        ) {
      ...
      prefetchNextPage: async () => {
        ...
        if (
          state.pageCache[next] &&
          Date.now() - state.pageCache[next].ts < state.pageCacheTTL
        )
          return
      ...
      searchProductByBarcodeFinancial: async barcode => {
        ...
        try {
          ...
          telemetry.record('products.search_financial_by_barcode.success', {
            ...
            canSell:
              product?.financial_health?.has_prices && product?.has_valid_stock,
      ...
      searchProductsFinancial: async (searchTerm, options = {}) => {
        ...
        try {
          ...
          telemetry.record('products.search_financial.success', {
            searchTerm,
            total: totalCount,
            withMargins: products.filter(p => p.best_margin_percent != null)
              .length,
            correlationId,
          })
          ...
          const norm = toApiError(
            error,
            'Error al buscar productos financieros',
            correlationId
          )
          const code =
            norm.code || (isConnectionError(error) ? 'NETWORK' : null)
          ...
          let errorMessage =
            norm.message || 'Error al buscar productos financieros'
          if (isConnectionError(error)) {
            errorMessage = getConnectionErrorMessage(error)
          }
        ...
        try {
          ...
          set(state => {
            const products = state.products.map(p =>
              p.id === productId ? { ...p, state: false } : p
            )
            const productsById = {
              ...state.productsById,
              [productId]: state.productsById[productId]
                ? { ...state.productsById[productId], state: false }
                : state.productsById[productId],
            }
            return {
              products,
              ...
              selectedProduct:
                state.selectedProduct?.id === productId
                  ? { ...state.selectedProduct, state: false }
                  : state.selectedProduct,
      ...
      reactivateProduct: async productId => {
        ...
        try {
          ...
          set(state => {
            const products = state.products.map(p =>
              p.id === productId ? { ...p, state: true } : p
            )
            const productsById = {
              ...state.productsById,
              [productId]: state.productsById[productId]
                ? { ...state.productsById[productId], state: true }
                : state.productsById[productId],
            }
            return {
              products,
              ...
              selectedProduct:
                state.selectedProduct?.id === productId
                  ? { ...state.selectedProduct, state: true }
                  : state.selectedProduct,
              loading: false,
            }
          })
          ...
          set({
            error: error.message || 'Error al reactivar producto',
            loading: false,
          })
          throw error
        }
      ...
      hydrateFromStorage: () => {
        try {
          const raw = window.localStorage.getItem(
            'products_last_offline_snapshot'
          )
          if (!raw) return null
          const parsed = JSON.parse(raw)
          set({
            products: parsed,
            productsById: Object.fromEntries(parsed.map(p => [p.id, p])),
      ...
      selectAllCurrent: () =>
        set(state => ({ selectedIds: state.products.map(p => p.id) })),
      ...
      bulkActivate: async () => {
        ...
        set(state => {
          ...
          ids.forEach(id => {
            if (productsById[id])
              productsById[id] = { ...productsById[id], is_active: true }
          })
          return { products, productsById }
        })
        try {
          await Promise.all(
            ids.map(id => productService.updateProduct(id, { is_active: true }))
          )
          telemetry.record('products.bulkActivate.success', {
            count: ids.length,
            correlationId,
          })
        } catch (e) {
          // Rollback
          set({
            productsById: prevSnapshot,
            products: Object.values(prevSnapshot).slice(
              0,
              get().products.length
            ),
          })
          telemetry.record('products.bulkActivate.rollback', {
            count: ids.length,
            correlationId,
          })
      ...
      bulkDeactivate: async () => {
        ...
        set(state => {
          ...
          ids.forEach(id => {
            if (productsById[id])
              productsById[id] = { ...productsById[id], is_active: false }
          })
          return { products, productsById }
        })
        try {
          await Promise.all(
            ids.map(id =>
              productService.updateProduct(id, { is_active: false })
            )
          )
          telemetry.record('products.bulkDeactivate.success', {
            count: ids.length,
            correlationId,
          })
        } catch (e) {
          ...
          telemetry.record('products.bulkDeactivate.rollback', {
            count: ids.length,
            correlationId,
          })
      ...
      optimisticUpdateProduct: async (id, patch) => {
        ...
        try {
          ...
          telemetry.record('products.inlineUpdate.success', {
            id,
            correlationId,
          })
          return true
          ...
          set(state => {
            ...
            return {
              productsById,
              products,
              error: 'Error al guardar cambios inline',
            }
          })
          telemetry.record('products.inlineUpdate.rollback', {
            id,
            correlationId,
          })
          return false
      ...
      _mapErrorCodeToHintKey: code => {
        switch (code) {
          case 'NETWORK':
            return 'errors.hint.NETWORK'
          case 'UNAUTHORIZED':
            return 'errors.hint.UNAUTHORIZED'
          case 'NOT_FOUND':
            return 'errors.hint.NOT_FOUND'
          case 'VALIDATION':
            return 'errors.hint.VALIDATION'
          case 'RATE_LIMIT':
            return 'errors.hint.RATE_LIMIT'
          case 'CONFLICT':
            return 'errors.hint.CONFLICT'
          case 'INTERNAL':
            return 'errors.hint.INTERNAL'
          default:
            return 'errors.hint.UNKNOWN'
        }
      },
      setTestingTTL: ms =>
      ...
      setTestingRetryConfig: cfg =>
        set(s => ({ _testing: { ...s._testing, overrideRetryConfig: cfg } })),
    }),
    {
      name: 'product-store',
      version: 2,
