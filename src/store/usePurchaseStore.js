  ...
  devtools(
    (set, get) => ({
      ...
      getPurchaseOrderAnalysis: async purchaseOrderId => {
        set({ loading: true, error: null })
        try {
          const response = await purchaseService.getPurchaseOrderAnalysis(
            purchaseOrderId
          )
          set({ loading: false })
          return response
      ...
      validatePurchaseOrderIntegrity: async purchaseOrderId => {
        set({ loading: true, error: null })
        try {
          const response = await purchaseService.validatePurchaseOrderIntegrity(
            purchaseOrderId
          )
          set({ loading: false })
          return response
      ...
      updateOrderStatus: async (id, newStatus, notes = '') => {
        set({ loading: true, error: null })
        try {
          const response = await purchaseService.updatePurchaseOrderStatus(
            id,
            newStatus,
            notes
          )
      ...
      setCurrentOrderSupplier: (supplierId, supplierName = '') => {
        ...
        telemetryService.recordEvent('purchase_supplier_selected', {
          ...
          newItems = [
            ...currentOrderData.items,
            {
              ...
              profit_pct:
                options.profit_pct ||
                product.profit_pct ||
                currentOrderData.default_profit_margin ||
                30,
      ...
      removeItemFromCurrentOrder: productId => {
        ...
        telemetryService.recordEvent('item_removed_from_purchase', {
          product_id: productId,
        })
      },
      ...
      previewPurchaseOrderCancellation: async purchaseOrderId => {
        ...
        try {
          const response =
            await purchaseService.previewPurchaseOrderCancellation(
              purchaseOrderId
            )
          set({ loading: false })
          return response
        ...
        try {
          ...
          if (response.success) {
            ...
            telemetryService.recordEvent(
              'purchase_order_cancelled_with_details',
              {
      ...
      updateCurrentOrderSettings: settings => {
        ...
        telemetryService.recordEvent(
          'purchase_order_settings_updated',
          settings
        )
      },
      ...
      searchOrders: async (searchTerm, searchType = 'supplier') => {
        set({ loading: true, error: null })
        try {
          const response = await purchaseService.searchPurchases(
            searchTerm,
            searchType
          )
      ...
      getCurrentOrderItemsCount: () => {
        return get().currentOrderData.items.reduce(
