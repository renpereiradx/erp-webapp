...
const SalePayment = () => {
  ...
  const getStatusBadge = status => {
    ...
    switch (s) {
      case 'PAID':
        return (
          <Badge className='bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border-none font-bold uppercase text-[10px] tracking-widest px-2 py-0.5'>
            Pagado
          </Badge>
        )
      case 'PARTIAL':
        return (
          <Badge className='bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-none font-bold uppercase text-[10px] tracking-widest px-2 py-0.5'>
            Parcial
          </Badge>
        )
      case 'CANCELLED':
        return (
          <Badge className='bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border-none font-bold uppercase text-[10px] tracking-widest px-2 py-0.5'>
            Cancelado
          </Badge>
        )
      case 'PENDING':
        return (
          <Badge className='bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400 border-none font-bold uppercase text-[10px] tracking-widest px-2 py-0.5'>
            Pendiente
          </Badge>
        )
      default:
  ...
  return (
    <div className='flex flex-col gap-4 md:gap-6 animate-in fade-in duration-500 max-w-[1600px] mx-auto py-2 px-4 md:px-0'>
      ...
      <header className='flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-l-4 border-primary pl-4 py-0.5'>
        <div>
          <h1 className='text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none'>
            Cobros de Ventas
          </h1>
        ...
        <Button
          ...
          className='h-9 border-slate-200 font-bold uppercase tracking-widest text-[11px] rounded-lg shadow-sm'
        >
          <RefreshCw
            size={14}
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
        {[
          ...
          <Card
            key={i}
            className='rounded-xl border-slate-100 dark:border-slate-800 shadow-fluent-2 bg-white dark:bg-surface-dark overflow-hidden transition-all duration-300'
          >
            <CardContent className='p-4 flex items-center gap-4'>
              <div
                className={`size-10 bg-${kpi.color}-500/10 rounded-xl flex items-center justify-center text-${kpi.color}-600`}
              >
                <kpi.icon size={20} />
              </div>
              <div>
                ...
                <h2 className='text-xl font-black text-slate-900 dark:text-white tracking-tight tabular-nums'>
                  {kpi.value}
                </h2>
              </div>
            </CardContent>
          ...
          <CardContent className='p-4 flex flex-col xl:flex-row items-end gap-4'>
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 flex-1 w-full'>
              <div className='space-y-1.5 min-w-0'>
                <label className='text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1 truncate block'>
                  Cliente
                </label>
            ...
            <>
                ...
                <Table className='min-w-[1000px]'>
                  <TableHeader>
                    <TableRow className='bg-transparent border-none hover:bg-transparent'>
                      <TableHead className='text-[10px] font-black uppercase tracking-widest text-slate-400 py-4 px-6'>
                        ID
                      </TableHead>
                      <TableHead className='text-[10px] font-black uppercase tracking-widest text-slate-400 px-4'>
                        Cliente
                      </TableHead>
                      <TableHead className='text-[10px] font-black uppercase tracking-widest text-slate-400 px-4'>
                        Fecha
                      </TableHead>
                      <TableHead className='text-[10px] font-black uppercase tracking-widest text-slate-400 px-4 text-center'>
                        Estado
                      </TableHead>
                      <TableHead className='text-[10px] font-black uppercase tracking-widest text-slate-400 px-4 text-right'>
                        Progreso
                      </TableHead>
                      <TableHead className='text-[10px] font-black uppercase tracking-widest text-slate-400 px-4 text-right'>
                        Total
                  ...
                  <TableBody className='divide-y divide-slate-50'>
                    {displaySales.map(sale => (
                      ...
                      >
                        <TableCell className='py-3.5 px-6 font-black text-sm'>
                          #{sale.id}
                        </TableCell>
                        ...
                        <TableCell className='px-4'>
                          <div className='flex flex-col items-end gap-1.5 w-24 ml-auto'>
                            <div className='flex items-center justify-between w-full text-[10px] font-bold text-slate-500'>
                              ...
                              {sale.payment_count > 0 && (
                                <span className='text-primary'>
                                  {sale.payment_count} pago
                        ...
                        <TableCell className='px-4 text-right font-bold text-sm tabular-nums'>
                          {formatCurrency(sale.total_amount)}
                        </TableCell>
                        <TableCell className='px-4 text-right font-black text-primary text-sm tabular-nums'>
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button
                                variant='ghost'
                                size='icon'
                                className='size-8 rounded-full text-slate-400 hover:text-primary hover:bg-primary/5'
                              >
                            ...
                            >
                              <DropdownMenuItem
                                ...
                                className='gap-3 py-2.5 font-bold text-xs rounded-md cursor-pointer'
                              >
                                <Eye size={16} /> Detalle
                              </DropdownMenuItem>
                              <DropdownMenuItem
                              ...
                              <DropdownMenuItem
                                ...
                                disabled={['paid', 'pagado'].includes(
                                  sale.status?.toLowerCase(),
                                )}
                                className='gap-3 py-2.5 font-bold text-xs rounded-md text-green-600 cursor-pointer hover:text-green-700 hover:bg-green-50'
                              >
              ...
              <div className='lg:hidden divide-y divide-slate-100'>
                {displaySales.map(sale => (
                  <div
                    key={sale.id}
                    className='p-4 space-y-4 active:bg-slate-50'
                    onClick={() => navigate(`/cobros-ventas/${sale.id}`)}
                  >
                    ...
                    >
                      <Button
                        ...
                        className='flex-1 h-9 bg-primary/10 text-primary hover:bg-primary hover:text-white border-none font-black uppercase tracking-widest text-[10px] rounded-md transition-all'
                      >
                        <CreditCard size={14} className='mr-2' /> Registrar
                        Cobro
                      </Button>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant='outline'
                            className='h-9 w-11 rounded-md border-slate-200'
                          >
                        ...
                        >
                          <DropdownMenuItem
                            ...
                            className='gap-3 py-3 font-bold text-xs rounded-md'
                          >
            ...
            <div className='px-4 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between'>
              ...
              <div className='flex gap-2'>
                <Button
                  ...
                  className='h-9 rounded-md border-slate-200 px-3'
                >
                  <ChevronLeft size={16} />
                </Button>
                <Button
