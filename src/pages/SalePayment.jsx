...
import { Receipt, CreditCard, Eye, X, Users, User, DollarSign, CheckCircle, AlertTriangle, Calendar, Package, RefreshCw, Clock, TrendingUp } from 'lucide-react';
...
const SalePayment = () => {
  ...
  const handleLoadCashRegisters = async () => {
    ...
    try {
      ...
      const openCashRegisters = allCashRegisters?.filter(cr => cr.status === 'OPEN' || cr.state === 'OPEN') || [];
      console.log('📦 Open cash registers:', openCashRegisters);

      setCashRegisters(openCashRegisters);
  ...
  const handleProcessPayment = async (e) => {
    ...
    try {
      ...
      if (paymentMode === 'cash_register') {
        ...
        result = await processSalePaymentWithCashRegister(paymentDataWithCashRegister);
      } else {
      ...
      if (result?.cash_summary) {
        ...
        if (result.requires_change && result.cash_summary.change_given > 0) {
          alert(
            ...
            `Detalles:\n` +
            `• Recibido: ${formatGuaranies(result.cash_summary.cash_received)}\n` +
            `• Aplicado a la venta: ${formatGuaranies(result.cash_summary.amount_applied)}\n` +
            `• Vuelto: ${formatGuaranies(result.cash_summary.change_given)}`
          );
        } else {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className={`p-3 ${styleConfig?.colors?.accent || "bg-primary"} rounded-lg shadow-sm`}>
        ...
        <div className="flex gap-3">
          ...
          >
            <RefreshCw className={`w-4 h-4 mr-2 ${isSalesLoading ? 'animate-spin' : ''}`} />
      ...
      <Card className={`${styles.card()} shadow-lg`}>
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          <div className="grid grid-cols-10 gap-4">
            ...
            <div className="col-span-3 space-y-2">
              <Label htmlFor="page_size_desktop" className="text-sm font-medium">
                Registros
              </Label>
              <Select
              ...
              >
                <SelectTrigger className={styles.input()} id="page_size_desktop">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <Card className={`${styles.card()} shadow-md`}>
        <CardHeader className="border-b py-3">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base">Filtros de Estado y Cliente</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="py-4">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            <div className="flex flex-wrap gap-2 flex-1">
              <Label className="text-sm font-medium w-full sm:w-auto">Estado de Venta</Label>
              <Button
                variant={filters.status === '' ? 'default' : 'outline'}
                size="sm"
                onClick={() => handleFilterByStatus('')}
                className={filters.status === '' ? styles.button("primary") : styles.button("secondary")}
                disabled={isSalesLoading}
              >
              ...
              <Button
                ...
                className={filters.status === 'PENDING' ? 'bg-yellow-600 hover:bg-yellow-700 text-white' : styles.button("secondary")}
                disabled={isSalesLoading}
              >
              ...
              <Button
                variant={filters.status === 'PARTIAL_PAYMENT' ? 'default' : 'outline'}
                size="sm"
                onClick={() => handleFilterByStatus('PARTIAL_PAYMENT')}
                className={filters.status === 'PARTIAL_PAYMENT' ? 'bg-orange-600 hover:bg-orange-700 text-white' : styles.button("secondary")}
                disabled={isSalesLoading}
              >
              ...
              <Button
                ...
                className={filters.status === 'COMPLETED' ? 'bg-green-600 hover:bg-green-700 text-white' : styles.button("secondary")}
                disabled={isSalesLoading}
              >
                <CheckCircle className="w-4 h-4 mr-2" />
                Completadas
              ...
              <Button
                ...
                className={filters.status === 'CANCELLED' ? 'bg-red-600 hover:bg-red-700 text-white' : styles.button("secondary")}
                disabled={isSalesLoading}
              >
            ...
            <div className="w-full sm:w-auto sm:min-w-[300px]">
              <Label className="text-sm font-medium block sm:hidden mb-2">Filtrar por Cliente</Label>
      ...
      <Card className={`${styles.card()} shadow-md`}>
        <CardHeader className="border-b py-3">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base">Ventas para Gestión de Pagos</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="pt-6">
          {isSalesLoading ? (
            <div className="text-center py-12">
              <div className={`inline-block animate-spin rounded-full h-12 w-12 border-b-2 ${styleConfig?.colors?.accent || "bg-primary"} mb-4`}></div>
            ...
            <div className="text-center py-12">
              ...
              <Button onClick={handleLoadSales} variant="outline" className={styles.button("secondary")}>
                Reintentar
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {sales.map((sale) => {
                ...
                return (
                  <Card key={sale.id} className={`${styles.card()} hover:shadow-lg transition-all duration-200`}>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-muted-foreground">
                              <strong>Fecha:</strong> {new Date(sale.sale_date || sale.created_at).toLocaleDateString('es-PY')}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <DollarSign className="w-4 h-4 text-muted-foreground" />
                            <span className="font-semibold">
                              <strong>Total:</strong> {formatGuaranies(totalAmount)}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                          ...
                          {(sale.status === 'PENDING' || sale.status === 'PARTIAL_PAYMENT') && (
                            <Button
                              size="sm"
            ...
            <div className="text-center py-16">
              <div className={`inline-block p-6 ${styleConfig?.colors?.accent || "bg-primary"} rounded-full mb-4`}>
                <Package className="w-16 h-16" />
              </div>
              <h3 className={`text-xl font-semibold mb-2`}>No hay ventas</h3>
              <p className={`${"text-muted-foreground"} mb-6 max-w-md mx-auto`}>
                Ajusta los filtros de fecha y haz clic en "Aplicar Filtros" para ver las ventas existentes
              </p>
              <Button onClick={handleLoadSales} className={styles.button("primary")}>
                <Package className="w-4 h-4 mr-2" />
                Cargar Ventas
              </Button>
            </div>
        ...
        <DialogContent className="!max-w-4xl">
          ...
          <form onSubmit={handleProcessPayment} className="space-y-6">
              ...
              <div className="bg-muted/40 p-4 rounded-lg border-l-4 border-primary">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{selectedSaleData.client_name}</p>
                  </div>
                  ...
                  {selectedSaleData.hasOwnProperty('balance_due') ? (
                    <>
                      ...
                      <div className="col-span-2 grid grid-cols-2 gap-3 border-t pt-3">
                        <div className="bg-gray-50 border border-gray-200 rounded p-3">
                          <span className="text-gray-700 font-semibold text-sm">Total Venta:</span>
                          <p className="text-2xl font-bold text-gray-900 mb-2">{formatGuaranies(selectedSaleData.total_amount || 0)}</p>
                          <div className="border-t border-gray-300 pt-2 mt-2">
                            <div className="flex items-center justify-between">
                              <span className="text-green-600 text-xs font-medium">Ya Pagado:</span>
                              <span className="font-semibold text-green-600 text-sm">{formatGuaranies(selectedSaleData.total_paid || 0)}</span>
                            </div>
                            {selectedSaleData.payment_count > 0 && (
                              <div className="text-xs text-muted-foreground mt-1">
                                ({selectedSaleData.payment_count} pago{selectedSaleData.payment_count > 1 ? 's' : ''} realizado{selectedSaleData.payment_count > 1 ? 's' : ''})
                              </div>
                            )}
                          </div>
                        </div>
                        <div className="bg-orange-50 border border-orange-200 rounded p-3">
                          <span className="text-orange-800 font-semibold text-sm">Pendiente a Pagar:</span>
                          <p className="text-2xl font-bold text-orange-600">{formatGuaranies(selectedSaleData.balance_due || 0)}</p>
                        </div>
                      </div>
                    </>
                  ) : (
                    <div className="col-span-2">
                      ...
                      <p className="text-2xl font-bold text-primary">{formatGuaranies(selectedSaleData.total_amount || 0)}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-3">
                ...
                <Select value={paymentMode} onValueChange={setPaymentMode}>
                  ...
                  <SelectContent>
                    <SelectItem value="cash_register" className="text-sm py-3 px-3">
                      <div className="flex items-center gap-2">
              ...
              <div className="space-y-3">
                <Label htmlFor="cash_register" className="text-sm font-semibold flex items-center gap-2">
                  <Package className="w-4 h-4" />
                  Caja Registradora {paymentMode === 'cash_register' && <span className="text-red-500">*</span>}
                </Label>
                <Select
                  value={selectedCashRegister?.toString()}
                  onValueChange={(value) => setSelectedCashRegister(parseInt(value))}
                  disabled={isCashRegistersLoading || cashRegisters.length === 0 || paymentMode !== 'cash_register'}
                >
                  <SelectTrigger className="h-11 text-sm px-4 py-3">
                  ...
                  <SelectContent>
                    {cashRegisters.map((cashRegister) => (
                      <SelectItem key={cashRegister.id} value={cashRegister.id.toString()} className="text-sm py-3 px-3">
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            Balance: {formatGuaranies(cashRegister.current_balance || 0)}
                          </div>
                        </div>
                      </SelectItem>
                    ))}
                ...
                {paymentMode === 'cash_register' && cashRegisters.length === 0 && !isCashRegistersLoading && (
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Monto Recibido */}
              <div className="space-y-3">
                <Label htmlFor="amount_received" className="text-sm font-semibold flex items-center gap-2">
                  <DollarSign className="w-4 h-4" />
              ...
              <div className="space-y-3">
                ...
                {selectedSaleData && paymentForm.amount_received && parseFloat(paymentForm.amount_received) > 0 ? (
                  <div className="h-14 flex items-center">
                    {(() => {
                      const amountReceived = parseFloat(paymentForm.amount_received);
                      // Usar balance_due si existe, sino usar total_amount
                      const amountDue = selectedSaleData.hasOwnProperty('balance_due')
                      ...
                      if (amountReceived >= amountDue) {
                        return (
                          <div className="w-full flex items-center justify-between border-amber-200 bg-amber-50 rounded-lg px-3 py-3 border-2">
                            <span className="font-semibold text-sm text-amber-900">Vuelto a entregar:</span>
                        ...
                        return (
                          <div className="w-full flex items-center justify-between text-blue-700 bg-blue-50 border-blue-200 rounded-lg px-3 py-3 border-2">
                            <span className="font-semibold text-sm">Quedarán pendientes:</span>
                  ...
                  <div className="h-14 flex items-center justify-center bg-gray-50 border border-gray-200 rounded-lg">
                    <span className="text-sm text-muted-foreground">Ingrese el monto recibido</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Payment Notes */}
              <div className="space-y-3">
                <Label htmlFor="payment_notes" className="text-sm font-semibold">
                  Nota del Pago
              ...
              {/* Payment Reference */}
              <div className="space-y-3">
                <Label htmlFor="payment_reference" className="text-sm font-semibold">
                  Referencia de Pago
                  <span className="text-xs font-normal text-muted-foreground ml-2">(Opcional)</span>
                </Label>
                <Input
      ...
      <Dialog open={saleDetailDialog} onOpenChange={setSaleDetailDialog}>
        <DialogContent className="max-w-[90vw] w-[1100px] max-h-[85vh] overflow-hidden flex flex-col">
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3 bg-muted/30 p-4 rounded-lg">
                          <h3 className="font-semibold text-lg border-b pb-2">Información de la Venta</h3>
                          <div className="space-y-2.5">
                            <p className="text-sm leading-relaxed">
                              <strong className="inline-block w-20">ID:</strong>
                              <span className="text-muted-foreground">{sale.sale_id || sale.id || 'N/A'}</span>
                            </p>
                            <p className="text-sm leading-relaxed">
                              <strong className="inline-block w-20">Cliente:</strong>
                              <span className="text-muted-foreground">{sale.client_name || 'N/A'}</span>
                            </p>
                            <p className="text-sm leading-relaxed">
                              <strong className="inline-block w-20">Fecha:</strong>
                              <span className="text-muted-foreground">
                                {sale.sale_date
                                  ? new Date(sale.sale_date).toLocaleDateString('es-PY', {
                                      year: 'numeric',
                                      month: '2-digit',
                                      day: '2-digit',
                                      hour: '2-digit',
                            ...
                            <p className="text-sm leading-relaxed flex items-center gap-2">
                              <strong className="inline-block w-20">Estado:</strong>
                              {getStatusBadge(sale.status?.toUpperCase() || 'PENDING')}
                            </p>
                          </div>
                        </div>
                        <div className="space-y-3 bg-muted/30 p-4 rounded-lg">
                          ...
                          <div className="space-y-2.5">
                            ...
                            {sale.hasOwnProperty('balance_due') && (
                              <>
                                <div className="border-t pt-2 mt-2">
                                  <p className="text-sm leading-relaxed">
                                    <strong className="inline-block w-32">Total Pagado:</strong>
                                    <span className="text-green-600 font-semibold">
                                      {formatGuaranies(sale.total_paid || 0)}
                                    </span>
                                  </p>
                                  <p className="text-sm leading-relaxed">
                                    <strong className="inline-block w-32">Balance Pendiente:</strong>
                                    <span className="text-orange-600 font-semibold">
                                      {formatGuaranies(sale.balance_due || 0)}
                                    </span>
                                  </p>
                                  <p className="text-sm leading-relaxed">
                                    <strong className="inline-block w-32">Progreso:</strong>
                                    <span className="text-muted-foreground">
                                      {sale.payment_progress?.toFixed(1)}% ({sale.payment_count || 0} pago{sale.payment_count !== 1 ? 's' : ''})
                                    </span>
                                  </p>
                                </div>
                                  ...
                                  <div className="w-full bg-gray-200 rounded-full h-3">
                                    <div
                                      className={`h-3 rounded-full transition-all ${
                                        sale.is_fully_paid ? 'bg-green-500' : 'bg-orange-500'
                                      }`}
                                      style={{ width: `${Math.min(sale.payment_progress || 0, 100)}%` }}
                                    />
                                  </div>
                                </div>
                              </>
                            ...
                            <div className="border-t pt-2 mt-2">
                              <p className="text-sm leading-relaxed">
                                <strong className="inline-block w-32">Usuario:</strong>
                                <span className="text-muted-foreground">{sale.user_name || 'N/A'}</span>
                              </p>
                              <p className="text-sm leading-relaxed">
                                <strong className="inline-block w-32">Método de Pago:</strong>
                                <span className="text-muted-foreground">{sale.payment_method || 'N/A'}</span>
                              </p>
                              {sale.currency && (
                                <p className="text-sm leading-relaxed">
                                  <strong className="inline-block w-32">Moneda:</strong>
                                  <span className="text-muted-foreground">{sale.currency}</span>
                                </p>
                              )}
                            </div>
                          </div>
                      ...
                      {sale.details && sale.details.length > 0 && (
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="text-sm">
                              {sale.items || sale.details.length} {(sale.items === 1 || sale.details.length === 1) ? 'producto' : 'productos'}
                            </Badge>
                          </div>
                            ...
                            <Table>
                              <TableHeader>
                                <TableRow className="bg-muted/50">
                                  ...
                                  <TableHead className="font-semibold text-right w-[140px]">Precio Base</TableHead>
                                  <TableHead className="font-semibold text-right w-[180px]">Precio Final</TableHead>
                                  <TableHead className="font-semibold text-right w-[120px]">Subtotal</TableHead>
                                  <TableHead className="font-semibold text-right w-[120px]">Total</TableHead>
                                </TableRow>
                              </TableHeader>
                              <TableBody>
                                  ...
                                  <TableRow key={detail.id || index}>
                                    <TableCell className="font-medium min-w-[200px]">
                                      <div>
                                        ...
                                        {detail.product_type && (
                                          <div className="text-xs text-muted-foreground">{detail.product_type}</div>
                                        )}
                                      </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="text-right w-[180px]">
                                      <div className="flex items-center justify-end gap-2">
                                        <span className="tabular-nums">{formatGuaranies(detail.unit_price)}</span>
                              ...
                              <div className="border-t pt-2 mt-2">
                                <div className="bg-primary/5 px-4 py-3 rounded-lg">
                                  <div className="flex justify-between items-center text-xl font-bold">
                                    <span>Total:</span>
                                    <span className="tabular-nums">{formatGuaranies(sale.total_amount || 0)}</span>
                                  </div>
                                </div>
                              </div>
                            </div>
          ...
          <DialogFooter className="flex-shrink-0 border-t pt-4 mt-4">
            <Button onClick={() => setSaleDetailDialog(false)} className={styles.button("primary")} size="lg">
              Cerrar
            </Button>
          </DialogFooter>
        </DialogContent>
