...
const EMPTY_KPI_FORM: Array<{ key: keyof AuditKPIs; labelKey: string; icon: string; iconClass: string }> = [
  { key: 'total_actions', labelKey: 'bi.audit.kpi.totalActions', icon: 'data_exploration', iconClass: 'bg-primary/10 text-primary' },
  { key: 'successful_actions', labelKey: 'bi.logs.kpi.successful', icon: 'check_circle', iconClass: 'bg-success/10 text-success' },
  { key: 'failed_actions', labelKey: 'bi.logs.kpi.failed', icon: 'error', iconClass: 'bg-error-container text-error' },
  { key: 'unique_users', labelKey: 'bi.audit.kpi.uniqueUsers', icon: 'person_search', iconClass: 'bg-warning/10 text-warning' },
];

// Fallbacks del t() (keys bi.audit.kpi.totalActions / bi.logs.kpi.* en locales)
const KPI_LABEL_FALLBACK: Record<string, string> = {
  total_actions: 'Acciones Totales',
  successful_actions: 'Éxitos',
  failed_actions: 'Fallos',
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 font-inter">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-foreground tracking-tight">{t('bi.logs.title', 'Registro de Auditoría')}</h1>
          <p className="text-on-surface-deep text-sm">{t('bi.logs.subtitle', 'Monitoreo avanzado de actividades del sistema en tiempo real.')}</p>
        </div>
        <button
      ...
      <div className="bg-surface p-4 rounded-xl border border-border-subtle shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-on-surface-deep uppercase tracking-wider">{t('bi.logs.filters.from', 'Desde')}</label>
            <input
              type="date"
              name="start_date"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-on-surface-deep uppercase tracking-wider">{t('bi.logs.filters.to', 'Hasta')}</label>
            <input
              type="date"
              name="end_date"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-on-surface-deep uppercase tracking-wider">{t('bi.audit.table.user', 'Usuario')}</label>
            <input
              type="text"
              name="search"
              ...
              onChange={handleFilterChange}
              className="w-full px-3 py-2 bg-surface-muted border border-border-subtle rounded-lg text-sm focus:ring-primary focus:border-primary transition-all outline-none text-foreground"
              placeholder={t('bi.logs.searchPlaceholder', 'Email o ID...')}
            />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-on-surface-deep uppercase tracking-wider">{t('bi.logs.filters.category', 'Categoría')}</label>
            <select
              name="category"
              ...
              className="w-full px-3 py-2 bg-surface-muted border border-border-subtle rounded-lg text-sm focus:ring-primary focus:border-primary transition-all outline-none text-foreground"
            >
              <option value="">{t("bi.logs.option.allCategories", "Todas")}</option>
              <option value="SALE">{t('bi.compare.crumb.sales', 'Ventas')}</option>
              <option value="AUTH">{t('bi.logs.option.auth', 'Autenticación')}</option>
              <option value="INVENTORY">{t('bi.logs.option.inventory', 'Inventario')}</option>
              <option value="PRODUCT">{t('bi.logs.option.products', 'Productos')}</option>
            </select>
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-on-surface-deep uppercase tracking-wider">{t('bi.logs.filters.level', 'Nivel')}</label>
            <select
              name="level"
              value={filters.level}
              onChange={handleFilterChange}
              className="w-full px-3 py-2 bg-surface-muted border border-border-subtle rounded-lg text-sm focus:ring-primary focus:border-primary transition-all outline-none text-foreground"
            >
              <option value="">{t("bi.logs.option.allLevels", "Todos")}</option>
              <option value="INFO">{t('bi.logs.option.info', 'Info')}</option>
              <option value="WARNING">{t('bi.logs.option.warning', 'Warning')}</option>
              <option value="ERROR">{t('bi.logs.option.error', 'Error')}</option>
            </select>
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-on-surface-deep uppercase tracking-wider">{t('bi.logs.filters.result', 'Resultado')}</label>
            <select
              name="success"
            ...
            >
              <option value="">{t("bi.logs.option.allLevels", "Todos")}</option>
              <option value="true">{t('bi.logs.option.success', 'Éxito')}</option>
              <option value="false">{t('bi.logs.option.failure', 'Fallo')}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-border-subtle shadow-sm overflow-hidden flex flex-col">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-muted border-b border-border-subtle">
                <th className="px-6 py-4 text-xs font-bold text-on-surface-deep uppercase tracking-wider w-48">{t('bi.logs.col.timestamp', 'Fecha y Hora')}</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-deep uppercase tracking-wider w-64">{t('bi.audit.table.user', 'Usuario')}</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-deep uppercase tracking-wider w-32">{t('bi.logs.col.action', 'Acción')}</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-deep uppercase tracking-wider w-48">{t('bi.logs.col.entity', 'Entidad')}</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-deep uppercase tracking-wider min-w-[300px]">{t('bi.logs.col.description', 'Descripción')}</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-deep uppercase tracking-wider w-32">{t('bi.logs.filters.level', 'Nivel')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border-subtle">
              {loading ? (
                <tr><td colSpan={6} className="text-center py-20 font-bold text-on-surface-deep uppercase tracking-widest animate-pulse">{t('bi.logs.loading', 'Cargando...')}</td></tr>
              ) : error ? (
                <tr>
                  <td colSpan={6} className="text-center py-20">
                    <p className="text-sm font-bold text-foreground">{t('bi.logs.loadError', 'No se pudieron cargar los registros de auditoría.')}</p>
                    <button
                      onClick={fetchLogs}
                ...
                <tr><td colSpan={6} className="text-center py-20 text-on-surface-deep">{t('bi.logs.empty', 'No se encontraron resultados')}</td></tr>
              ) : (
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-surface-muted transition-colors group cursor-pointer" onClick={() => navigate(`/auditoria/logs/${log.id}`)}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-primary">{log.entity_id || t('bi.common.na', 'N/A')}</span>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 bg-surface-muted border-t border-border-subtle">
          <div className="flex items-center gap-4">
            <span className="text-sm text-on-surface-deep">{t('bi.logs.rowsPerPage', 'Filas por página:')}</span>
            <select
              value={pageSize}
            ...
            <span className="text-sm text-on-surface-deep font-medium">{total.toLocaleString('es-PY')} {t('bi.logs.records', 'registros')}</span>
          </div>
          <div className="flex items-center gap-6">
            <p className="text-sm text-on-surface-deep font-medium">{t('bi.logs.page', 'Página')} <span className="font-black text-foreground">{page}</span> {t('bi.logs.of', 'de')} <span className="font-black text-foreground">{totalPages}</span></p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
        {EMPTY_KPI_FORM.map((stat) => (
          <div key={stat.key} className="bg-surface p-4 rounded-lg border border-border-subtle shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black text-on-surface-deep uppercase tracking-wider">{t(stat.labelKey, KPI_LABEL_FALLBACK[stat.key])}</p>
