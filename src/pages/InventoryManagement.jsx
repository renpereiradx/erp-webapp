...
import { Filter, Calendar, Download, Plus, ChevronLeft, ChevronRight, MoreHorizontal, X, Trash2, Search, Loader2 } from 'lucide-react';
...
const InventoryManagement = () => {
  ...
  const handleProductSearch = useCallback(async (searchTerm) => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const activeProducts = resultsArray.filter(product =>
  ...
  const handleSubmitInventory = async () => {
    ...
    if (!inventoryData.items || inventoryData.items.length === 0) {
      setFormErrors(['Error: No hay productos para enviar. Por favor, agregue al menos un producto.']);
      return;
    }
    ...
    if (result.success) {
      ...
      toast.success(
        `Inventario creado exitosamente con ${inventoryData.items.length} producto${inventoryData.items.length !== 1 ? 's' : ''}`,
        4000
      );
  ...
  return (
    <div className="inventory-management">
                ...
                return (
                  <tr key={inventory.id}>
                    ...
                    <td className="inventory-management__cell">
                      <span
                        className={`inventory-management__badge ${getStatusClass(status.type)}`}
                      >
                        {status.label}
                      </span>
                    </td>
                ...
                <div className="inventory-management__pagination">
                  ...
                  <div className="inventory-management__pagination-controls">
                    ...
                    {getPageNumbers().map((page, index) => (
                      page === '...' ? (
                        <span key={`ellipsis-${index}`} className="inventory-management__pagination-ellipsis">
                          ...
                        </span>
                      ) : (
                        <button
                          ...
                          className={`inventory-management__pagination-number ${
                            pagination.page === page ? 'inventory-management__pagination-number--active' : ''
                          }`}
      ...
      {showDeleteConfirm && (
        <div className="inventory-management__modal-overlay" onClick={() => setShowDeleteConfirm(null)}>
          <div className="inventory-management__modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="inventory-management__modal-footer">
              ...
              <button
                ...
                disabled={loading}
              >
                {loading ? 'Procesando...' : t('inventoryManagement.deleteModal.confirm')}
              </button>
            </div>
          </div>
        </div>
      ...
      {showCreateModal && (
        <div className="inventory-management__modal-overlay" onClick={() => setShowCreateModal(false)}>
          <div className="inventory-management__modal inventory-management__modal--large" onClick={(e) => e.stopPropagation()}>
            <div className="inventory-management__modal-header">
              <h2 className="inventory-management__modal-title">
              ...
              {formErrors.length > 0 && (
                <div className="inventory-management__form-errors">
                  {formErrors.map((error, index) => (
                    <div key={index} className="inventory-management__form-error">
                      {error}
                    </div>
                  ))}
                </div>
                ...
                <div className="inventory-management__form-grid">
                  ...
                  <div className="inventory-management__form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => handleMetadataChange('verification', e.target.value)}
                    >
                      <option value="single_check">{t('inventoryManagement.createModal.verificationSingle')}</option>
                      <option value="double_check">{t('inventoryManagement.createModal.verificationDouble')}</option>
                    </select>
                  </div>
                </div>
                <div className="inventory-management__form-group">
                  ...
                  <textarea
                    ...
                    placeholder={t('inventoryManagement.createModal.notesPlaceholder')}
              ...
              <div className="inventory-management__details-section">
                ...
                <div className="inventory-management__product-search-container">
                  <div className="inventory-management__search-input-wrapper">
                    ...
                    />
                    {searchLoading && (
                      <Loader2 size={20} className="inventory-management__search-loader" />
                    )}
                  </div>
                    ...
                    <div ref={dropdownRef} className="inventory-management__search-dropdown">
                      {searchLoading ? (
                        <div className="inventory-management__search-dropdown-item inventory-management__search-dropdown-item--loading">
                          {t('inventoryManagement.createModal.searchingProducts')}
                        </div>
                ...
                {inventoryForm.items.length === 0 ? (
                  ...
                  <div className="inventory-management__products-table-wrapper">
                    <table className="inventory-management__products-table">
                      <thead>
                        <tr>
                          <th>{t('inventoryManagement.createModal.productId')}</th>
                          <th>{t('inventoryManagement.createModal.productName')}</th>
                          <th>{t('inventoryManagement.createModal.currentQuantity')}</th>
                          <th>{t('inventoryManagement.createModal.quantityCounted')}</th>
                          ...
                          <tr key={index}>
                            ...
                            <td>
                              <button
                                ...
                                type="button"
                                title={t('inventoryManagement.createModal.remove')}
                              >
                                <Trash2 size={18} />
                              </button>
                            </td>
      ...
      {showDetailsModal && selectedInventory && (
        <div className="inventory-management__modal-overlay" onClick={() => { setShowDetailsModal(false); clearSelectedInventory(); }}>
          <div className="inventory-management__modal inventory-management__modal--large" onClick={(e) => e.stopPropagation()}>
            <div className="inventory-management__modal-header">
              <h2 className="inventory-management__modal-title">
              ...
              <div className="inventory-management__details-section">
                ...
                <div className="inventory-management__details-grid">
                  ...
                  <div className="inventory-management__details-item">
                    ...
                    <div className="inventory-management__details-value">
                      {selectedInventory.inventory.metadata?.counting_method || 'N/A'}
                    </div>
                  </div>
                  <div className="inventory-management__details-item">
                    <div className="inventory-management__details-label">
                    ...
                    <div className="inventory-management__details-value">
                      {selectedInventory.inventory.metadata?.verification || 'N/A'}
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <table className="inventory-management__items-table">
                    <thead>
                      <tr>
                        ...
                        <th>{t('inventoryManagement.viewModal.previousQuantity')}</th>
                        <th>{t('inventoryManagement.viewModal.quantityChecked')}</th>
                        <th>{t('inventoryManagement.viewModal.difference')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedInventory.items.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>{item.quantity_checked - item.previous_quantity}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
