...
const ProductCard = React.memo(({ product, onView, onEdit, onDelete, onReactivate, onToggleSelect, selected, inlineEditing, onStartInlineEdit, onCancelInlineEdit, onInlineSave }) => {
  ...
  return (
    <div className={`transition-all duration-200 hover:-translate-y-1 hover:shadow-fluent-8 active:translate-y-0 active:shadow-fluent-2 h-full ${styles.card(cardVariant || 'group flex flex-col', cardVariant ? { interactive: true, density: 'compact', extra: 'group flex flex-col' } : '')} ${isInactive ? 'opacity-70 border-dashed border-2' : ''}`}>
