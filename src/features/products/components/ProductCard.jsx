...
const ProductCard = React.memo(({ product, onView, onEdit, onDelete, onToggleSelect, selected, inlineEditing, onStartInlineEdit, onCancelInlineEdit, onInlineSave }) => {
  ...
  return (
    <div className={`${styles.card('group flex flex-col')} transition-all duration-200 hover:-translate-y-0.5 focus-within:ring-2 focus-within:ring-ring`}>
      <div className="flex-1 p-4">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">{t('field.category')}</span><span>{productSummary.category?.name}</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">{t('field.price')}</span><span className="font-semibold text-green-600">{productSummary.priceFormatted || 'N/A'}</span></div>
      ...
      <div className="flex gap-2 p-4 border-t border-border/50">
        ...
        <Button onClick={() => onEdit?.(product)} variant="secondary" size="sm" className="flex-1 text-xs"><Edit className="w-4 h-4 mr-1" />{t('action.edit')}</Button>
