/**
 * ReservationsStep — paso condicional del SaleCheckoutWizard.
 *
 * Aparece cuando el cliente tiene reservas confirmadas. Migra ReservationModal:
 * toggle por reserva con la regla de negocio "una reserva por producto"
 * (mismo product_id). El operador puede omitir o añadir las seleccionadas.
 */
...
export const ReservationsStep = forwardRef<ReservationsStepRef, ReservationsStepProps>(
  ({ pendingReservations, selectedResIds, onToggleSelection, formatDateTime }, ref) => {
    const { t } = useI18n()
      ...
      <div className="space-y-4">
        ...
        <div ref={listRef} className="space-y-2 max-h-[360px] overflow-y-auto pr-1">
          {pendingReservations.map((res, index) => {
            ...
            // Regla: una reserva por producto.
            ...
            const blocked = isProductAlreadySelected && !isSelected
              ...
              >
                ...
                {blocked && (
                  <div className="mt-2 text-[10px] font-bold uppercase text-on-surface-variant bg-surface-container rounded px-2 py-1 inline-block">
                    {t('sales.checkoutWizard.reservations.productAlreadySelected', 'Producto ya seleccionado')}
                  </div>
                )}
              </button>
            )
