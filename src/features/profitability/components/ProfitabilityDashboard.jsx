  ...
  return (
    <div className='bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:border-[#137fec] group'>
      <div className='flex flex-col gap-3'>
        <p className='text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.15em] leading-none'>
          {title}
        </p>

        <div className='flex flex-col'>
          <h3
            className={`min-w-0 font-black text-slate-900 dark:text-white font-mono tracking-tighter leading-tight whitespace-nowrap ${fontSizeClass}`}
          >
            {displayValue}
          </h3>

          <div
            className={`flex items-center gap-1 mt-2 px-2 py-1 rounded-lg w-fit ${bgTrendClass} ${colorClass} text-[10px] font-black uppercase tracking-widest`}
          >
...
const ProfitabilityDashboard = () => {
  ...
  if (loading)
    return (
      <div className='p-16 text-center animate-pulse text-slate-500 font-bold uppercase tracking-widest text-[10px]'>
        Modelando proyecciones de rentabilidad...
      </div>
    )
  if (error)
    return (
      <div className='p-10 text-center text-rose-500 bg-rose-50 border border-rose-100 rounded-2xl m-6 font-black uppercase tracking-tight text-xs'>
        Fallo en la carga del dashboard: {error}
      </div>
    )
      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-6'>
        <div className='flex flex-col gap-1'>
          ...
          <p className='text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1'>
            Control de márgenes operativos y rendimiento corporativo (Gs.)
          </p>
        </div>
        ...
        <div className='col-span-12 lg:col-span-8 bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden group'>
          <div className='flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-4'>
            <div>
              ...
              <p className='text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2'>
                Relación Ingresos vs Beneficio Real
              </p>
            </div>
          ...
          <div className='h-72 flex items-end justify-between px-4 bg-slate-50/30 dark:bg-slate-950/20 rounded-[1.5rem] p-6 gap-2'>
            {[60, 75, 65, 85, 90, 70, 80, 95].map((h, i) => (
              <div
                key={i}
                className='flex-1 max-w-[40px] bg-[#137fec]/10 rounded-t-xl relative group/bar'
                style={{ height: `${h}%` }}
              >
                <div
        ...
        <div className='col-span-12 lg:col-span-4 space-y-6'>
          <div className='bg-[#137fec] p-8 rounded-[2rem] text-white shadow-2xl shadow-blue-500/20 relative overflow-hidden group'>
            ...
            <div className='flex justify-between items-start mb-6'>
              <h3 className='font-black text-xs uppercase tracking-[0.3em] leading-none'>
                Punto de Equilibrio
              </h3>
              <Target
            ...
            <div className='space-y-6'>
              ...
              <div className='grid grid-cols-2 gap-6'>
                <div>
                  ...
                  <p className='font-black text-sm uppercase tracking-tighter mt-1'>
                    {break_even_status.performance_status === 'ABOVE_BREAK_EVEN'
                      ? 'SUPERADO'
                      : 'PENDIENTE'}
                  </p>
                </div>
                <div>
                  ...
                  <p className='font-black text-sm font-mono mt-1'>
                    +{kpis.roi}%
                  </p>
                </div>
              </div>
          ...
          <div className='bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm group'>
            ...
            <div className='space-y-4'>
              {alerts.map((alert, i) => (
                <div
                  key={i}
                  className='flex gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl transition-all hover:translate-x-1'
                >
                  <div
                    className={`px-2 py-1 h-fit rounded text-[8px] font-black uppercase tracking-widest text-white ${alert.severity === 'HIGH' ? 'bg-rose-500 shadow-lg shadow-rose-500/20' : 'bg-blue-500 shadow-lg shadow-blue-500/20'}`}
                  >
                    {alert.severity || 'INFO'}
                  </div>
                  <div className='flex flex-col gap-1'>
                    <p className='text-[10px] font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight leading-none'>
                      {alert.type}
                    </p>
