          ...
          <TableHeader>
            <TableRow>
              ...
              <th className='text-right pr-6'>
                {t('receivables.overdue.table.actions')}
              </th>
            </TableRow>
          </TableHeader>
          <TableBody>
            {safeAccounts.map((acc, idx) => (
              <TableRow key={idx} className='group'>
                ...
                <TableCell>
                  <div className='overdue-accounts__client-cell'>
                    <div
                      ...
                      style={{ backgroundColor: acc.bgColor || '#eff6ff' }}
                    >
                      {acc.code}
                    </div>
                    <div className='overdue-accounts__client-info'>
                      <span className='overdue-accounts__client-info-name'>
                        {acc.client}
                      </span>
                      <span className='overdue-accounts__client-info-id'>
                        {acc.id}
                      </span>
                    </div>
                  </div>
                ...
                <TableCell className='text-center'>
                  <span
                    className={`status-pill ${acc.priority === 'High' ? 'status-pill--danger' : 'status-pill--warning'}`}
                  >
                    {acc.days}
                  </span>
                </TableCell>
                <TableCell>
                  <span
                    className={`rec-badge ${acc.priority === 'High' ? 'rec-badge--danger' : acc.priority === 'Medium' ? 'rec-badge--warning' : 'rec-badge--success'}`}
                  >
                    {acc.priority === 'High'
                      ? 'Prioridad Alta'
                      : acc.priority === 'Medium'
