...
> **Nota (2026-07-08):** Todos los endpoints ahora incluyen `metadata.scope`
> indicando si la cifra es consolidada (todas las sucursales) o de una sucursal
> específica. Ver [VARIANT_AWARE_BI.md](VARIANT_AWARE_BI.md). Además, los KPIs de
> inventario y los márgenes (`profit`) son ahora variant-aware: el stock efectivo
> suma padre + variantes activas, y el COGS usa el costo de la variante de cada
> línea de venta.
>
...
Todos los endpoints requieren JWT Bearer Token: `Authorization: Bearer <token>`.
...
Cada respuesta incluye `metadata.scope`:
...
```bash
curl -X GET "http://localhost:5050/dashboard/summary?period=today" \
...
```bash
curl -X GET "http://localhost:5050/dashboard/alerts?severity=critical" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json"
```
