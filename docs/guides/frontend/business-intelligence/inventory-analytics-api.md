...
|-----------|------|-------------|
| `branch_id` | int (query) | Filtra por sucursal. Si se omite: un Admin ve el total **consolidado** (todas las sucursales); un usuario sin permisos multi-sucursal recibe `400 branch context required`. |
| `X-Branch-ID` | int (header) | Alternativa a `?branch_id`. |

Cada respuesta incluye `metadata.scope` indicando el alcance:

```json
"metadata": {
  "generated_at": "...",
  "scope": { "consolidated": false, "branch_id": 1 }
}
```

- `consolidated: true` → la cifra agrega todas las sucursales.
- `consolidated: false` + `branch_id` → la cifra es de esa sucursal.

> Ver [VARIANT_AWARE_BI.md](VARIANT_AWARE_BI.md) para el detalle de integridad de
> datos con variantes, multi-sucursal y dimensiones de producto.
...
| `brand_id` | int | Filtrar por marca (`products.brand_id`) |
| `tag` | string (repetible) | Filtrar por slug de etiqueta. Selecciona productos con AL MENOS una de las etiquetas (semántica OR). Ej: `?tag=verano&tag=oferta` |
...
> **Nota sobre `sku`:** se expone usando el `id` del producto como string.
>
> **Nota sobre `current_stock` (variant-aware):** cuando un producto tiene
> variantes activas, `current_stock` es el **stock efectivo** = fila padre
> (`variant_id IS NULL`) + suma de variantes activas. Cada producto aparece
> **una sola vez** en el listado, sin importar cuántas variantes tenga.
>
> **Campos opcionales de dimensión:** `brand_id`, `brand_name` y `tags` se
> omiten (`omitempty`) cuando el producto no tiene marca o etiquetas asignadas.
