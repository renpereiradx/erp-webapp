...
> **Variant-aware (2026-07-08):** el costo de ventas (COGS) en
> `income-statement`, `profit-margins` y `overview` ahora usa el costo de la
> variante de cada línea de venta (`get_cost_for_variant`), con fallback al
> producto padre. El valor de inventario (`inventory_data`) usa stock efectivo
> (padre + variantes activas).
