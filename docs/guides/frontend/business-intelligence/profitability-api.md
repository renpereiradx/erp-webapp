...
> **COGS por variante:** `cost`, `gross_profit`, `gross_margin_pct`, `markup`,
> `contribution_pct` y `profit_per_unit` resuelven el costo por línea de venta
> usando `get_cost_for_variant(product_id, variant_id)` — cada venta con
> `variant_id` usa el costo de su variante (con fallback al padre).
