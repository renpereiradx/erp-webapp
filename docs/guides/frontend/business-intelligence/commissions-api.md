...
curl -X GET "http://localhost:5050/commissions/plans?active_only=true" \
  -H "Authorization: Bearer <token>"

# Asignar plan a vendedor
curl -X POST "http://localhost:5050/commissions/assignments" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"seller_id": "user-123", "plan_id": 1}'

# Obtener resumen de comisiones
curl -X GET "http://localhost:5050/commissions/summary?period=month" \
  -H "Authorization: Bearer <token>"

# Obtener comisiones de un vendedor
curl -X GET "http://localhost:5050/commissions/seller/user-123?period=month&status=PENDING" \
  -H "Authorization: Bearer <token>"

# Aprobar comisiones
curl -X POST "http://localhost:5050/commissions/approve" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"commission_ids": [1, 2, 3]}'

# Obtener dashboard
curl -X GET "http://localhost:5050/commissions/dashboard?period=month" \
  -H "Authorization: Bearer <token>"
```
