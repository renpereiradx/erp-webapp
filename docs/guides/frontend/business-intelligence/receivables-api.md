...
```bash
curl -X GET "http://localhost:5050/receivables/overview" \
...
```bash
curl -X GET "http://localhost:5050/receivables?status=OVERDUE&sort_by=days_overdue&sort_order=desc" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json"
```
...
```bash
curl -X GET "http://localhost:5050/receivables/client/client_123/risk" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json"
```
...
```bash
curl -X GET "http://localhost:5050/receivables/aging/report" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json"
```
