...
Cada respuesta incluye `metadata.scope` indicando el alcance (`consolidated` o
`branch_id`). Ver [VARIANT_AWARE_BI.md](VARIANT_AWARE_BI.md) para el detalle.
...
>
> **Nota sobre `cost`/`gross_profit` (variant-aware):** cada línea de venta
> resuelve su costo con la función `get_cost_for_variant(product_id, variant_id)`.
> Si la línea de venta tiene `variant_id` y esa variante tiene costo propio en
> `unit_costs`, se usa ese costo; si no, se usa el costo del producto padre.
> Por eso, los márgenes son correctos por variante.
>
> **Campos opcionales de dimensión:** `brand_id`, `brand_name` y `tags` se
> omiten (`omitempty`) cuando no aplican.
...
curl -X GET "http://localhost:5050/sales-analytics/performance?period=month&compare=true" \
  -H "Authorization: Bearer <token>"

# Obtener tendencias por rango de fechas
curl -X GET "http://localhost:5050/sales-analytics/trends/date-range?start_date=2026-01-01&end_date=2026-01-15&granularity=daily" \
  -H "Authorization: Bearer <token>"

# Obtener dashboard ejecutivo
curl -X GET "http://localhost:5050/sales-analytics/dashboard?period=month" \
  -H "Authorization: Bearer <token>"
```
