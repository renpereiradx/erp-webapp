...
**Estado:** Vigente (multi-branch, unit conversion)
...
## Conversión de Unidad en Compras

Cuando el `unit` del detalle de compra es diferente al `base_unit` del producto, el sistema **convierte automáticamente** la cantidad a `base_unit` antes de afectar el stock.

### Comportamiento

| Escenario | Ejemplo | Resultado en stock |
|-----------|---------|-------------------|
| `unit` = `base_unit` | Compra 10 kg de Papa (`base_unit=kg`) | stock += 10 kg |
| `unit` ≠ `base_unit` (conversión existe) | Compra 1 box de Papa (`base_unit=kg`, conversión box→kg=20) | stock += 20 kg |
| `unit` ≠ `base_unit` (sin conversión) | Compra 1 box de Papa sin conversión registrada | **Error: transacción rechazada** |

### Error cuando falta conversión

Si no existe un factor de conversión entre la unidad de compra y el `base_unit` del producto, la transacción se rechaza con:
...
**Solución:** Registrar la conversión vía `POST /unit-conversions` antes de intentar la compra. Ver [UNIT_CONVERSIONS_API_GUIDE.md](./UNIT_CONVERSIONS_API_GUIDE.md) para ejemplos de conversiones de empaque.

### Trazabilidad de Unidad en Stock

El sistema registra la unidad original de la transacción en el metadata de `stock_transactions`:
...
```json
{
  ...
  "original_unit": "box",
  "original_quantity": 1,
  "converted_quantity": 20,
  "base_unit": "kg"
}
```

Esto permite al dashboard mostrar: *"Se compró 1 caja (convertido a 20 kg)"*.

## Precio Derivado en base_unit

Cuando `auto_update_prices=true` y la compra fue en una unidad diferente al `base_unit`, el sistema **deriva automáticamente** el precio en `base_unit` y lo registra en `unit_prices`.

### Ejemplo

```
Producto: Papa (base_unit = kg)
Compra: 1 box a Gs. 40.000
Conversión: box → kg = 20
Margen: 30%
auto_update_prices: true

Resultado:
- Precio box = 40.000 × 1.30 = Gs. 52.000  → unit_prices(PAPA, box, 52000)
- Precio kg  = 52.000 / 20   = Gs. 2.600   → unit_prices(PAPA, kg, 2600) ← derivado
```

### Warnings

Si no existe conversión para derivar el precio, se agrega un warning al response (sin fallar la transacción):

```json
{
  "warnings": [
    {
      "type": "PRICE_DERIVATION_SKIPPED",
      "product_id": "PAPA",
      "reason": "No se pudo derivar precio en kg: conversion no disponible"
    }
  ]
}
```
