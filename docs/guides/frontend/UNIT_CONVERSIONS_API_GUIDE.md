...
### v1.1.0 - 5 de Junio de 2026
- **NUEVO**: Endpoint `GET /unit-conversions/template` con ejemplos de conversiones de empaque comunes.
...
### 4. Plantilla de Conversiones

**`GET /unit-conversions/template`**

Retorna ejemplos de conversiones de empaque comunes para guiar al administrador. **No** crea las conversiones — solo muestra ejemplos para que el admin las cargue vía `POST /unit-conversions`.

**Permisos:** `products:read` (solo lectura).
  ...
  "template": [
    {
      ...
      "to_unit": "kg",
      "factor": "20",
      "example": "1 caja de papas = 20 kg"
    },
    {
      "from_unit": "bag",
      "to_unit": "kg",
      "factor": "5",
      "example": "1 bolsa de arroz = 5 kg"
    },
    {
      "from_unit": "case",
      "to_unit": "l",
      "factor": "12",
      "example": "1 cajon de gaseosas = 12 litros"
    },
    {
      "from_unit": "bundle",
      "to_unit": "kg",
      "factor": "10",
      "example": "1 atado de lena = 10 kg"
    },
    {
      "from_unit": "dozen",
      "to_unit": "unit",
      "factor": "12",
      "example": "1 docena de huevos = 12 unidades"
    }
  ]
}
```

**Uso recomendado:** El frontend puede mostrar estos ejemplos en un dropdown o tabla de referencia cuando el admin crea conversiones de empaque.
...
Las conversiones de empaque (`box→kg`, `bag→kg`, `case→l`, etc.) **NO** vienen pre-cargadas en el sistema. El administrador debe cargarlas manualmente vía `POST /unit-conversions`.

### ¿Por qué no vienen pre-cargadas?

El factor de conversión depende del negocio:
- "1 caja de papas" puede ser 20 kg para un proveedor y 25 kg para otro
- "1 bolsa de arroz" puede ser 5 kg o 10 kg según el empaque

### Flujo recomendado

1. Consultar `GET /unit-conversions/template` para ver ejemplos
2. Adaptar los factores según la realidad del negocio
3. Cargar vía `POST /unit-conversions`

### Impacto si no se cargan

Las transacciones (compras y ventas) que usen unidades de empaque se **rechazarán** con error `NO_CONVERSION` hasta que se registre la conversión correspondiente.

---

## Errores de Conversión en Transacciones

Cuando una compra o venta usa una unidad diferente al `base_unit` del producto y no existe un factor de conversión, el sistema rechaza la transacción con un mensaje claro:

### En Compras (`POST /purchase/complete`)

```json
{
  "success": false,
  "message": "No existe conversion de box a kg para el producto PAPA (Papa). Registre la conversion en POST /unit-conversions"
}
```
...
Luego reintentar la transacción.
