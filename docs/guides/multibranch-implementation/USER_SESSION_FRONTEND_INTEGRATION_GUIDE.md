...
### GET /sessions/active
**Descripción:** Obtiene las sesiones activas del usuario actual con datos enriquecidos del usuario.

#### Response 200
| Campo | Tipo | Descripción |
|-------|------|-------------|
| success | bool | `true` |
| data | object[] | Lista de sesiones activas enriquecidas |
| count | int | Cantidad de sesiones |

#### Session Object (enriquecido)
| Campo | Tipo | Descripción |
|-------|------|-------------|
| id | int | ID de la sesión |
| user_id | string | ID del usuario |
| token_hash | string | Hash del token (solo referencia interna) |
| ip_address | string | Dirección IP de la sesión |
| user_agent | string | User-Agent del cliente |
| device_type | string | Tipo de dispositivo (`mobile`, `desktop`, `tablet`, `unknown`) |
| is_active | bool | Si la sesión está activa |
| last_activity | string | Timestamp ISO de última actividad |
| expires_at | string | Timestamp ISO de expiración |
| created_at | string | Timestamp ISO de creación |
| revoked_at | string | Timestamp ISO de revocación (si aplica) |
| revoked_by | string | ID del usuario que revocó (si aplica) |
| revoke_reason | string | Razón de revocación (si aplica) |
| **user_first_name** | string | Nombre del usuario |
| **user_last_name** | string | Apellido del usuario |
| **user_username** | string | Username del usuario |
| **user_email** | string | Email del usuario |

---

### GET /sessions/history
**Descripción:** Obtiene el historial completo de sesiones del usuario (activas e inactivas) con datos enriquecidos del usuario.
...
|-------|------|-------------|
| success | bool | `true` |
| data | object[] | Lista de sesiones (mismo schema que `/sessions/active`) |
| page | int | Página actual |
| page_size | int | Items por página |
| count | int | Cantidad de sesiones en la página |
...
|-------|------|-------------|
| success | bool | `true` |
| data | object[] | Lista de sesiones activas de todos los usuarios (mismo schema enriquecido que `/sessions/active`) |
| count | int | Cantidad total de sesiones activas |
